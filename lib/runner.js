...
module.exports = function MigrationRunner(options){
  ...
  function ensureMigrationTableExists(callback){
    Step(
      ...
      function extention1() {
        db.query("CREATE EXTENSION IF NOT EXISTS btree_gist;", this);
      },
      function extention2() {
        db.query("CREATE EXTENSION IF NOT EXISTS bdr;", this);
      },
      function extention3() {
        db.query("CREATE EXTENSION IF NOT EXISTS \"uuid-ossp\";", this);
      },
      function createTableIfNecessary(err, result){
        if (err) return callback(err);
        if (result && result.rows && result.rows.length == 1) return callback();
        db.query('CREATE TABLE '+options.migrations_table+' (id UUID PRIMARY KEY DEFAULT uuid_generate_v1mc(), name varchar(255) NOT NULL, run_on timestamp NOT NULL)', this);
      },
      function finish(err){
        if (err) return callback(err);
        return callback();
